...
class LibThorVGRecipe(MesonRecipe):
    ...
    def build_arch(self, arch):
        ...
        with current_directory(build_dir):
            ...
            import os
            pattern1 = join(self.ctx.ndk.llvm_prebuilt_dir, "lib/clang/*/lib/linux", lib_arch)
            pattern2 = join(self.ctx.ndk.llvm_prebuilt_dir, "lib64/clang/*/lib/linux", lib_arch)
            clang_lib_dir_list = glob(pattern1) + glob(pattern2)
            if clang_lib_dir_list:
                clang_lib_dir = clang_lib_dir_list[0]
